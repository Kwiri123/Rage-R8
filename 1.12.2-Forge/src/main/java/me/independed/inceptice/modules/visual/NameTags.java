...
public class NameTags extends Module {
    ...
    public static void drawRect(float f, float f2, float f3, float f4, int n) {
        ...
        GL11.glEnd();
        NameTags.glColor(new Color(255, 255, 255, 255).getRGB());
        GL11.glPopMatrix();
    ...
    void renderNameTag(EntityLivingBase entityLivingBase, String string, double d, double d2, double d3) {
        RenderManager string2;
        if (entityLivingBase instanceof EntityArmorStand || entityLivingBase == NameTags.mc.player) {
            return;
        }
        ColorUtils.color(0, 0, 0, 255);
        EntityPlayerSP entityPlayerSP = NameTags.mc.player;
        FontRenderer fontRenderer = NameTags.mc.fontRenderer;
    ...
    public void onRenderWorldLast(float f) {
        if (NameTags.mc.player == null || NameTags.mc.player.isDead) {
            return;
        }
        List list = NameTags.mc.world.loadedEntityList.stream().filter(entity -> entity != NameTags.mc.player).filter(entity -> !entity.isDead).sorted(Comparator.comparing(entity -> Float.valueOf(NameTags.mc.player.getDistance(entity)))).collect(Collectors.toList());
        for (Object entity2w : list) {
            ...
            double d5;
            double d6;
            if (entity2 == null || entity2 == NameTags.mc.player) continue;
            if (this.invisibleSet.isEnabled() && entity2.isInvisible() && (entity2 instanceof EntityPlayer && this.playerSet.isEnabled() || entity2 instanceof EntityMob && this.mobSet.isEnabled() || entity2 instanceof EntityAnimal && this.animalSet.isEnabled())) {
                d6 = NameTags.mc.getRenderManager().viewerPosX;
                d5 = NameTags.mc.getRenderManager().viewerPosY;
                d4 = NameTags.mc.getRenderManager().viewerPosZ;
