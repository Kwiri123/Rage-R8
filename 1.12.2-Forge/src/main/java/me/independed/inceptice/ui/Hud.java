...
public class Hud
    ...
    private boolean getClosest1(Entity entity) {
    ...
    private boolean getClosest2(Entity entity) {
    ...
    @SubscribeEvent
    public void renderOverlay(RenderGameOverlayEvent renderGameOverlayEvent) throws IOException {
        ScaledResolution scaledResolution = new ScaledResolution(this.mc);
        ...
        if (ModuleManager.getModuleByName("HUD").isToggled() && renderGameOverlayEvent.getType() == RenderGameOverlayEvent.ElementType.TEXT) {
            ...
            for (Object modulew : ModuleManager.getSortedModules()) {
                Module module = (Module) modulew;
                if (!module.isToggled() || module.getName().equals("TabGUI") || module.getName().equals("Gui")) continue;
                this.toAdd = "no";
                for (Object settingw : module.settings) {
                    Setting setting = (Setting) settingw;
    ...
    private Float getClosest3(Entity entity) {
        return Float.valueOf(this.mc.player.getDistance(entity));
    }
}
